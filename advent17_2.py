...
# # start by going right by 4, hardcoded
# queue = [(12, 0, 4, 'r')]  # dist, cur_i, cur_j, direc
# dist_dict = {(0, 4): {'r': 12}}

# # start by going down by 4, hardcoded
# queue = [(13, 4, 0, 'd')]  # dist, cur_i, cur_j, direc
# dist_dict = {(4, 0): {'d': 13}}
...
while queue:
    ...
    if cur_direc != 'r' and cur_direc != 'l':
        ...
        for move in range(1, 4):
            if cur_j + move >= ncol:
                break
            new_dist += grid[cur_i][cur_j + move]
        for move in range(4, 11):
            if cur_j + move >= ncol:
                break
    ...
    if cur_direc != 'u' and cur_direc != 'd':
        ...
        for move in range(1, 4):
            if cur_i - move < 0:
                break
            new_dist += grid[cur_i - move][cur_j]
        for move in range(4, 11):
            if cur_i - move < 0:
                break
