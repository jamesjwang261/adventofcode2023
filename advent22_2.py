# brute force just works so its what i went with
...
block_dict_orig = copy.deepcopy(block_dict)
grid_orig = copy.deepcopy(grid)


total = 0
for bad_num in bad_remove:
    block_dict = copy.deepcopy(block_dict_orig)
    grid = copy.deepcopy(grid_orig)
    
    # remove block bad_num
    (lx, ly, lz), (rx, ry, rz) = block_dict[bad_num]
    for x in range(lx, rx+1):
        for y in range(ly, ry+1):
            for z in range(lz, rz+1):
                grid[x][y][z] = 0
    del block_dict[bad_num]
    
    # do all the falling again
    for sz in range(1, maxz+1):
        for sx in range(maxx+1):
            for sy in range(maxy+1):
                if grid[sx][sy][sz] != 0:
    ...
    for block_num in block_dict:
        if block_dict[block_num] != block_dict_orig[block_num]:
            fall += 1
    # print(bad_num, fall)
    total += fall
